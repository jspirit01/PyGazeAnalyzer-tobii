...
def draw_heatmap(fixations, dispsize, imagefile=None, durationweight=True, alpha=0.5, savefilename=None):
	...
	strt = int(gwh/2)    #TypeError: 'float' object cannot be interpreted as an integer 방지하기 위함
	heatmapsize = dispsize[1] + 2*strt, dispsize[0] + 2*strt 		
	print(heatmapsize)
...
def draw_display(dispsize, imagefile=None):
	...
	if imagefile != None:
		...
		w, h = len(img[0]), len(img)	# (1025, 669)
		# x and y position of the image on the display
		x = int(dispsize[0]/2 - w/2)			# must be integer!!! (안 그럼 에러남)
		y = int(dispsize[1]/2 - h/2)			# must be integer!!!
